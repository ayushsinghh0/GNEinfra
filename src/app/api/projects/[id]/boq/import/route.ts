...
// POST /api/projects/[id]/boq/import — bulk-create BOQ items from an uploaded
// .xlsx (multipart form-data, field "file").
export async function POST(
  ...
  try {
    ...
    const { rows, skipped } = await parseBoqWorkbook(buffer, id);
    if (rows.length === 0) {
